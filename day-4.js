...
const NOT_REQUIERED_FIELD = 'cid';

const VALIDATIONS = {
    'byr': byr => {
        const validData = { length: 4, min: 1920, max: 2002 }
        return byr.length === validData.length && +byr >= validData.min && +byr <= validData.max;
    },
    'iyr': iyr => {
        const validData = { length: 4, min: 2010, max: 2020 };
        return iyr.length === validData.length && +iyr >= validData.min && +iyr <= validData.max;
    },
    'eyr': eyr => {
        const validData = { length: 4, min: 2020, max: 2030 };
        return eyr.length === validData.length && +eyr >= validData.min && +eyr <= validData.max;
    },
    'hgt': hgt => {
        const hgtLen = hgt.length;
        const messeureUnit = `${hgt.charAt(hgtLen - 2)}${hgt.charAt(hgtLen - 1)}`;
        const validData = {
            'cm': { min: 150, max: 193 }, 'in': { min: 59, max: 76 }
        }

        if (!validData[messeureUnit]) {
            return false;
        };

        const [value] = hgt.split(messeureUnit);
        return +value >= validData[messeureUnit].min && +value <= validData[messeureUnit].max
    },
    'hcl': (hcl) => {
        const validData = { rule: /^#[a-f0-9]{6}$/i };
        return hcl.match(validData.rule)
    },
    'ecl': (ecl) => {
    ...
    'pid': (pid) => {
        const validData = { length: 9 };
        return pid.length === validData.length;
    },
    'cid': () => true
}

const parsePazzleInput = (pazzleInput) => {
    const splitLines = pazzleInput.split("\n\n").map(w => w.replace(/\n/g, " "));
...
const passports = parsePazzleInput(pazzleInput);

// PART ONE 

const totalValidPassports = (passports) => {
    const validPassports = passports.filter(p => {
        ...
        if (keys.length < 7) {
            ...
            return !keys.includes(NOT_REQUIERED_FIELD);
        } else {
            return true
        }
    });
...
const totalValidatedPassportsWithStrictRules = passports => {
    const validPassports = totalValidPassports(passports);
    return validPassports.filter(p => {
        const passportKeys = Object.keys(p);
        return passportKeys.every(key => {
            const isValid = VALIDATIONS[key];
            return isValid(p[key]);
        })
    });
}

const totalValidPassportsResult = totalValidatedPassportsWithStrictRules(passports)
console.log('TOTAL', totalValidPassportsResult.length)