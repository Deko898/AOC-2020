...
const groupInput = input => {
    return input.split('\n').reduce((acc, curr) => {
        if (curr === '') {
            ...
            acc[acc.length - 1].subGrp.answers.push(curr)
        }
        return acc;
    }, []);
}

const parsedInput = groupInput(input);

const removeDuplicates = items => [...new Set(items)];
const flatenItems = items => items.map(item => item.split('')).flat();

// PART ONE

const countPositiveAnswers = answers => {
    return answers.reduce((acc, answer) => {
        const flatenAnswers = flatenItems(answer.subGrp.answers);
        const uniqueAnswers = removeDuplicates(flatenAnswers);
        return acc += uniqueAnswers.length
    }, 0)
}

// console.log(countPositiveAnswers(parsedInput));


// PART TWO

const countEveryMemebrPositiveAnswer = answers => {

    return answers.reduce((acc, answer) => {
        const grp = answer.subGrp.answers;
        const totalMembers = grp.length;
        const flatenAnswers = flatenItems(grp);

        const groupByAnswer = flatenAnswers.reduce((prev, answer) => {
            return {
                ...prev,
                [answer]: prev[answer] ? prev[answer] += 1 : 1
            }
        }, {
            totalMembers
        });
        let totalCorrect = 0
        for (const key in groupByAnswer) {
            if (groupByAnswer[key] === groupByAnswer.totalMembers && key !== 'totalMembers') {
                totalCorrect++
            }
        };
...
console.log(countEveryMemebrPositiveAnswer(parsedInput))