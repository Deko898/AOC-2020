...
const validatePasswords = (passwords) => {
    ...
    const validPasswords = passwords.filter(pass => {
        const [policy] = Object.keys(pass); // '4-15 b'
        const [minMax, char] = policy.split(' '); // minMax=4-15 char=b
        const [min, max] = minMax.split('-'); // min=4 max=15
        const totalIncludes = pass[policy].split('').reduce((prev, curr) => {
            if (curr === char) {
                prev++
            }
            return prev
        ...
        return totalIncludes >= min && totalIncludes <= max
    })

    return validPasswords.length
}
console.log(validatePasswords(data));


// PART TWO

const validatePasswordsByIndexPos = (passwords) => {
    // [{ '1-3 a': 'abcde' }]
    const validPasswords = passwords.filter(pass => {
        const [policy] = Object.keys(pass); // '1-3 a'
        const [minMax, char] = policy.split(' '); // minMax = 1-3; char = a;
        const [min, max] = minMax.split('-'); // min = 1 max = 3;

        const passwordLetters = pass[policy].split(''); // [a,b,c,d,e]

        const firstCondition = passwordLetters[+min - 1] === char;

        if (firstCondition) {
            return passwordLetters[+max - 1] !== char;
        }else {
            return passwordLetters[+max - 1] === char
        }
    })
...
console.log(validatePasswordsByIndexPos(data))