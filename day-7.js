const bagRules = `shiny lime bags contain 3 muted magenta bags, 3 clear cyan bags.
...
const partTwoMock = `shiny gold bags contain 2 dark red bags.
...
const { removeDuplicates } = require('./utils/util');

const brules = bagRules.split('\n');


// PART ONE 

const findBagsThatCanHoldSpecificColor = (bagsToFind, bagRules, total) => {
    const result = bagRules
        .filter(rule => bagsToFind.some(word => rule.includes(word)))
        .filter(rule => bagsToFind.every(word => !rule.startsWith(word)))
        .map(rule => {
            const [ten, color] = rule.split(' ');
            return `${ten} ${color}`
        });
    ...
    return findBagsThatCanHoldSpecificColor(result, bagRules, total);
}

const total = findBagsThatCanHoldSpecificColor(['shiny gold'], brules, []);

// console.log('total', removeDuplicates(total).length)


// PART TWO
const getTotalBagsAllowed = (bagToFind) => {
    let totalBags = 0
    const countTotalBags = (bagToFind, count = 1) => {
        const bagRule = brules.find(rule => rule.startsWith(bagToFind));
        if (!bagRule) {
            return
        }
        const [_, rule] = bagRule.split('contain');
        const splitAndTrim = rule.trim().split(',').map(x => x.trim());

        const mapedBags = splitAndTrim.map(x => {
            let [total, ...color] = x.split(' ');
            if (!isNaN(total)) {
                total *= count;
                totalBags += total
            }
            return {
                total, color: color.join(' ').replace('.', '')
            }
        })

        mapedBags.forEach(bag => countTotalBags(bag.color, bag.total));
    }
    countTotalBags(bagToFind);
    return totalBags;
}

console.log(getTotalBagsAllowed('shiny gold'), 'AAA')